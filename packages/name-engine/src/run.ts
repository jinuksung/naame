...
async function resolveDataPath(): Promise<string> {
  ...
  if (envSourcePath) {
    if (await fileExists(envSourcePath)) {
      return envSourcePath;
    }
    throw new Error(`[run] DATA_SOURCE_PATH 파일이 존재하지 않습니다: ${envSourcePath}`);
  }

  const legacyCsvPath = process.env.DATA_CSV_PATH?.trim();
  if (legacyCsvPath) {
    if (await fileExists(legacyCsvPath)) {
      console.warn("[run] DATA_CSV_PATH는 deprecated 입니다. DATA_SOURCE_PATH 사용을 권장합니다.");
      return legacyCsvPath;
    }
    throw new Error(`[run] DATA_CSV_PATH 파일이 존재하지 않습니다: ${legacyCsvPath}`);
  }

  if (await fileExists(DEFAULT_SOURCE_PATH)) {
  ...
  const normalizedMatch = jsonlFiles.find((entry) => entry.normalize("NFC") === normalizedDefault);
  if (normalizedMatch) {
  ...
  if (fuzzyMatch) {
    console.warn(`[run] 기본 JSONL 경로 미존재. fallback 파일 사용: ${fuzzyMatch}`);
    return fuzzyMatch;
  }

  throw new Error(
    `[run] JSONL 파일을 찾지 못했습니다. DATA_SOURCE_PATH를 지정하거나 ${DEFAULT_SOURCE_PATH} 파일을 추가하세요.`
  );
}
...
async function main(): Promise<void> {
  ...
  console.info(`[run] 데이터 로드 시작: ${dataPath}`);
  ...
  const lowRealityCandidates = ["가노", "노가", "고노", "가지", "가나", "고지", "규도"];
