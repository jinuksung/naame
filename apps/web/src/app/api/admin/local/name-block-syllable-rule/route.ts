...
import { addNameBlockSyllableRule } from "@/server/ssotAdmin";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await addNameBlockSyllableRule(nameHangul);
    return NextResponse.json({ ok: true, inserted: result.inserted });
  } catch (error) {
    console.error("[admin] name-block-syllable-rule failed", error);
    return NextResponse.json(
      { error: "Failed to add syllable block rule" },
      { status: 500 },
    );
  }
}
