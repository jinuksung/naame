...
export async function generateMetadata({
  ...
  return buildSeoMetadata({
    ...
    description: `${surname}씨 ${label} 이름 추천 키워드로 이름 후보를 비교하는 네임핏 랜딩입니다. 성씨 결합, 생년월일 입력 흐름, 관련 내부 링크를 통해 추천 단계를 자연스럽게 이어갈 수 있습니다.`,
...
export default function ComboLandingPage({
  ...
  const faqItems: FaqItem[] = [
    {
      ...
      answer: `${surname}씨 ${label} 조건을 동시에 보고 싶은 사용자에게 적합합니다. 성씨와 성별을 함께 고정한 상태에서 후보를 이해하고 내부 링크로 다른 조합과 비교할 수 있습니다.`,
    },
    {
  ...
  return (
    <SeoPageShell
      ...
      description={`${surname}씨와 ${label} 기준을 함께 고려한 추천 흐름과 대표 샘플을 확인할 수 있습니다.`}
    >
      <ParagraphSection
        title={`${surname}씨 ${label} 조합 추천 흐름`}
        paragraphs={[
          `${surname}씨 ${label} 조합 페이지는 조건을 좁혀서 이름 후보를 비교하려는 사용자에게 맞춘 랜딩입니다. 성씨와 성별을 동시에 고정하면 추천 설명을 읽을 때 기준이 명확해져 후보를 빠르게 정리할 수 있습니다. 페이지에서 제공하는 샘플은 조합 기준으로 고정되어 있어 다시 방문해도 동일한 목록을 바탕으로 비교를 이어갈 수 있습니다.`,
