...
const FEATURE_LIKED_PAGE_PATH = path.resolve(__dirname, "..", "feature", "liked", "page.tsx");
...
function testLikedPagesExistWithEmptyStateCopy(): void {
  assert.equal(
    existsSync(LIKED_PAGE_PATH),
    true,
    "토스 앱에 찜 목록 화면(/liked)이 있어야 합니다.",
  );
  assert.equal(
    existsSync(FEATURE_LIKED_PAGE_PATH),
    true,
    "토스 앱에는 feature 경로 별칭(/feature/liked)이 있어야 합니다.",
  );
  ...
  assert.equal(
    source.includes('className="liked-list"'),
    true,
    "찜 목록은 결과 화면(result-list)과 분리된 일반 카드 리스트(liked-list)로 렌더링되어야 합니다.",
  );
  assert.equal(
    source.includes('className="result-list"'),
    false,
    "찜 목록은 result-list를 사용하면 첫 카드 강조 스타일이 적용되므로 사용하면 안 됩니다.",
  );
  assert.equal(
...
function run(): void {
  ...
  testLikedPagesExistWithEmptyStateCopy();
  console.log("[test:liked-page-layout:toss] all tests passed");
}
